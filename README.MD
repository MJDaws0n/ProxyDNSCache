...
## How to setup auto renew for wildcard domains with cloudlfare and certbot
  1.  If you have certbot installed remove it (this is as it's most likley outdated)
      ```bash
      sudo apt-get remove certbot
      sudo apt autoremove
      ```
  3.  Follow these instructions to install certbot using python for linux (do the respective for windows and mac) https://certbot.eff.org/instructions?ws=other&os=pip
  4.  Install cloudflare plugin
      ```bash
      # Enter python vertual environment
...
2) Paste the following (minimal) service definition:
...
- `config.yml` and `cache.json` are expected in `WorkingDirectory` (and will be auto-created if missing).
- If you want to store them elsewhere, set `CONFIG_PATH` / `CACHE_PATH` environment variables.
- Binding to ports 80/443 typically requires root or capabilities. If you run as non-root, you can grant the binary permission to bind privileged ports:
